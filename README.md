...
Updating the code requires manual comparison with the original repository: https://github.com/docker-library/python/.
Usually it requires updating `versions.sh` or `Dockerfile-linux.template`. 



publishing new versions:
* create a directory with the version number
* run `update.sh` - the script will populate the folders Dockerfiles
